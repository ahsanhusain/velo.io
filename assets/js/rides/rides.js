...
function _table(dt = 'booked') {
    window.localStorage.setItem('table', dt);
    ...
    table = $('#'+dt).DataTable({
        ...
        "pageLength": 10,
        "language": {
            "lengthMenu": "<h1>Schedule Rides</h1>",
            "search": "",
            "info": "",
            "paginate": {
                "previous": "",
...
var which_table = window.localStorage.getItem('table') || 'booked';
_table(which_table);
...
function loadtable(tab = 'booked'){
     window.localStorage.setItem('table', tab);
     location.reload();
}
